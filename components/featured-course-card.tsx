      ...
      <div className="hidden md:block absolute inset-0 w-full h-full">
        ...
        {course.isFree && (
          <div className="absolute top-4 right-4 z-20">
            <span className="bg-green-800 text-green-lima text-xs font-bold px-3 py-1 rounded-full">Gratís</span>
          </div>
        )}
        <div className="absolute inset-0 flex flex-col justify-end p-6 z-10">
          ...
          <Link href={`/cursos/${course.id}`}>
            <Button
              className={
                course.isFree
                  ? "bg-green-800 text-green-lima font-bold w-full md:w-auto rounded-full"
                  : "bg-orange-700 hover:bg-orange-500 text-black w-full md:w-auto rounded-full"
              }
            >
              {course.isFree ? "Gratis" : "Acceder al curso"}
