...
interface EKitProductPageProps {
  params: {
    category: string
    id: string
  }
...
export default function EKitProductPage({ params }: EKitProductPageProps) {
  const router = useRouter()
  const { isAuthenticated, user } = useAuthContext()
  const { currency, formatPrice, isLoading: currencyLoading, error: currencyError } = useCurrency()

  const [product, setProduct] = useState<EKitProduct | null>(null)
  const [relatedProducts, setRelatedProducts] = useState<EKitProduct[]>([])
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        ...
        const productData = await api.get<EKitProduct>(`/product/${productId}`)
        setProduct(productData)

        // Obtener productos relacionados de la misma categoría
        if (apiCategory) {
          const allProducts = await api.get<EKitProduct[]>(`/product/category/e-kit/${apiCategory}`)
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      if (!isAuthenticated) {
        ...
        const redirectUrl = `/checkout?ekitId=${productId}`
      ...
      // Preparar datos del e-kit para el componente de checkout
      const ekitData = {
        id: product.id,
        name: product.name,
        price: finalPrice,
        ...
        format: "digital", // Los e-kits son siempre digitales
        deliveryType: "digital", // Formato en inglés para el backend
        type: "product", // Cambiado de "e-kit" a "product"
      ...
      localStorage.setItem("checkoutProduct", JSON.stringify(ekitData))
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl">Cargando e-kit...</p>
        </div>
      </div>
    )
  }
  ...
  if (!product) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <p className="text-xl">E-Kit no encontrado</p>
      </div>
    )
  }
          ...
          <div className="lg:col-span-2">
            <div className="mb-8">
              <div className={`${getCategoryColor(category)} font-medium mb-2`}>
                E-KIT {displayCategory.toUpperCase()}
              </div>
              ...
              {/* Contenido del E-Kit */}
              <div className="mb-8">
                <h2 className="text-xl font-bold mb-4">Contenido del e-kit</h2>
              ...
              {hasPurchased ? (
                <div className="space-y-4">
                  <div className="bg-green-50 p-4 rounded-md text-green-800 flex items-center gap-2">
                    ...
                    <span>Ya tienes acceso a este e-kit</span>
                  </div>
                  <Button className={`w-full ${getButtonColor(category)}`}>Descargar e-kit</Button>
                </div>
              ) : (
                <>
                  {/* Precio */}
    ...
    <>
      ...
      <span>Comprar</span>
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-16">
            <h2 className="text-2xl font-bold mb-8">Más e-kits de {displayCategory}</h2>
                  ...
                  <div key={relatedProduct.id} className="bg-white rounded-lg overflow-hidden shadow">
                    ...
                    <div className="p-4">
                          ...
                          <>
                            {relatedProduct.isFree ? (
                              <div className="flex items-center">
                                <span className="text-2xl font-bold text-green-600">Gratís</span>
                              </div>
                            ) : (
                              <>
                                <div className="flex items-center">
                      ...
                      <Button
                        ...
                        onClick={() => router.push(`/productos/e-kit/${category}/${relatedProduct.id}`)}
                        disabled={currencyLoading}
                      >
                        <ShoppingCart className="mr-2 h-4 w-4" />
                        {relatedProduct.isFree ? "Gratís" : "Comprar"}
                      </Button>
                    </div>
                  </div>
                )
